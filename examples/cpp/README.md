...
then build changed sources with CMake before running it. If the project has not
been configured yet, a Ninja debug build is created under `build` automatically.
Projects with multiple test executables remain ambiguous; for those, open
`<leader>ts`, mark a group with `m`, and press `C` to select its executable
manually.
